...
def startJob(intent, session):
    ...
    if rawJobName:
        ...
        jobName = rawJobName #jenkins.searchJobByName(rawJobName)
        if not jobName:
            ...
            speech_output = "The job is queued for running " 
            should_end_session = False
    else:
...
def getJobStatus(intent, session):
    ...
    if rawJobName:
        ...
        r = jenkins.getJobStatus('TestHello')#(jobName)
...
def getSpeechOutputForStatus(response):
    ...
    if res["result"] == "SUCCESS":
        return "The build finished succesfully " 
...
def lambda_handler(event, context):
    ...
    if (event['session']['application']['applicationId'] !=
            "amzn1.ask.skill.280326d3-3db5-41e5-8759-d0fa8a32f0d9"):
